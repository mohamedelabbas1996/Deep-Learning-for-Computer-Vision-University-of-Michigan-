...
def compute_distances_two_loops(x_train, x_test):
  ...
  x_train_reshaped = x_train.reshape(num_train, reduce(lambda x,y: x*y, x_train.shape[1:]))
  x_test_reshaped = x_test.reshape(num_test, reduce(lambda x,y: x*y, x_test.shape[1:]))
...
def compute_distances_one_loop(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  x_train_flattened = x_train.reshape(num_train , torch.numel(x_train)//num_train)
  x_test_flattened = x_test.reshape(num_test , torch.numel(x_test)//num_test)

  #(num_test, x)
  # (1, x) 
  for i in range(num_train):
    dists[i] = torch.sum( torch.square(x_test_flattened - x_train_flattened[i]), dim = 1 )
...
def compute_distances_no_loops(x_train, x_test):
  ...
  x_train_flattened = x_train.reshape(num_train , torch.numel(x_train)//num_train)
  x_test_flattened = x_test.reshape(num_test , torch.numel(x_test)//num_test)
  x_train_dots =torch.sum (x_train_flattened * x_train_flattened, dim=1) .reshape (num_train,1).repeat(1,num_test)
  x_test_dots =torch.sum (x_test_flattened * x_test_flattened, dim=1) .reshape (1,num_test).repeat(num_train,1)
  dists = x_train_dots + x_test_dots - 2*x_train_flattened.mm(x_test_flattened.t()) 
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  for j in range(dists.shape[1]): 
    #print ('column = ',dists[:,j])
    top_k_return = torch.topk(dists[:,j], int(k),largest=False )
    top_k_indices = top_k_return.indices
    top_k_labels = [y_train[i] for i in top_k_indices]
    #print ('top k =',torch.topk(dists[:,j], 3 ))
    #print ('labels = ',top_k_labels)
    
    
    pred = torch.argmax(torch.bincount(torch.tensor(top_k_labels).to(torch.int64)))
    #print('pred = ',pred)
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    for i in range(len(x_train_folds)):
      x_before_i = torch.cat(x_train_folds[:i]) if i > 0 else torch.tensor([]) 
      x_after_i =  torch.cat(x_train_folds[i+1:]) if i <  len(x_train_folds) -1 else torch.tensor([])
      y_before_i = torch.cat(y_train_folds[:i]) if i > 0 else torch.tensor([]) 
      y_after_i =  torch.cat(y_train_folds[i+1:]) if i <  len(x_train_folds) -1 else torch.tensor([])
      
      cls = KnnClassifier(torch.cat((x_before_i,x_after_i)),torch.cat((y_before_i,y_after_i)))

      acc = cls.check_accuracy(x_train_folds[i],y_train_folds[i], k = k)
...
def knn_get_best_k(k_to_accuracies):
  ...
  k_mean_acc = [ [key,torch.tensor(value).mean()] for key ,value in k_to_accuracies.items()]

  best_k = k_mean_acc[torch.argmax(torch.tensor([i[1] for i in k_mean_acc]))][0]
