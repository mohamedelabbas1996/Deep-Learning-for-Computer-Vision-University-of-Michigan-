...
def hello_linear_classifier():
  ...
  """
  print('Hello from linear_classifier.py after modification!')
...
def svm_loss_vectorized(W, X, y, reg):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  scores = X.mm(W)# W (C,D) , X (N,D) => (N,C)
  correct_class_score =  scores[torch.arange(scores.shape[0]),y] # => (N,1)
  #print ("scores dim  = ",scores.shape)
  #print ("correct _score dim = ", correct_class_score.shape)
  diff =  scores - correct_class_score.reshape(X.shape[0],1)+1
  #print ("diff = ",diff.shape)
  margin = torch.maximum(diff,torch.zeros_like(diff) ) # (N,C)
  ...
  margin_gt_zero = margin
  margin_gt_zero[margin>0] =1 
  row_sum = torch.sum(margin_gt_zero,dim=1) 
  margin_gt_zero[torch.arange(num_train),y] = - row_sum.t() 
  dW = torch.mm(X.t(),margin_gt_zero) 
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  dW/= num_train
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  train_acc = (y_pred == data_dict['y_train']).float().sum()/data_dict['X_train'].shape[0]
  y_val_pred = cls.predict(data_dict['X_val'])
  val_acc =  (y_val_pred == data_dict['y_val']).float().sum()/data_dict['X_val'].shape[0]
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(X.shape[0]):
    scores = W.t().mv(X[i]) #(C,D) . (1,D) = (1,C)
    scores -= torch.max(scores) #

    #softmax_output = torch.exp(scores)/torch.sum(torch.exp(scores)) #(1,C)
    #correct = torch.zeros(C,1)[y[i]:] = 1
    sum_i = 0.0
    for j in range(C):
      sum_i += torch.exp(scores)[j]
    loss = - scores[y[i]] + torch.log(sum_i)    
